...
$show_all_results = $_SESSION['show_all_interviews'];
...
$eids = array_keys(\REDCap::getEventNames());
$eid1 = $project->firstEventId;
...
foreach($data as $rid => $record) {
	...
	if (!empty($eids)) {
		...
		$enrollment_timestamp = strtotime($record[$eid1][$enrollment_field_name]);
	}
	...
	if ($show_all_results) {
		...
		foreach ($interviews as $interview) {
			...
			$entry = [];
			$entry["name"] = $seq_name;
			$entry["scheduled_datetime"] = $interview->db_timestamp;
			$entry["days_to_complete"] = $days_to_complete;
			$entry["date_to_complete"] = date("Y-m-d H:i", strtotime("+$days_to_complete days", strtotime($interview->db_timestamp)));
			$entry["interview"] = $interview;
			$entry["status"] = $interview->status;
			$sequences[] = $entry;
		}
	} else {
	...
	foreach ($sequences as $i => $seq) {
		...
		} elseif (!empty($seq['interview'])) {
			$interview = $seq['interview'];
		} else {
		...
		// add subjectid column if configured
		if ($add_columns) {
